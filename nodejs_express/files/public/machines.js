...
const activeMachineImage = {
  expressions: '.\server-machine-active.svg',
  animals: '.\server-machine-animals.svg'
};

const activeSingularRouteImage = {
  expressions: '.\expression-route-active.svg',
  animals: '.\animal-route-active.svg'
};

const activePluralRouteImage = {
  expressions: '.\expressions-route-active.svg',
  animals: '.\animals-route-active.svg'
};
  ...
  $.ajax('/', {
    success: function() {
      ...
      if ($('#expression-route').attr('src') === activeSingularRouteImage[getInactiveMode()] ||
           $('#expression-route').attr('src') === ".\expression-route-inactive.svg") {
    ...
    error: function(xhr) {
      if (xhr.status !== 404) {
        ...
        $('#expression-route').attr('src', '.\expression-route-inactive.svg');
        $('#expressions-route').attr('src', '.\expressions-route-inactive.svg');
      }
    }
  });
}
...
function activateMachine() {
  ...
  $('#beaker').attr('src', '.\beaker-active-neutral.svg');

  if ($('#lightbulb').attr('src') === '.\lightbulb-inactive.svg') {
    $('#lightbulb').attr('src', '.\lightbulb-active.svg');
  }
}
...
function deactivateMachine() {
  ...
  $('#server-machine').attr('src', '.\server-machine-inactive.svg');
  $('#lightbulb').attr('src', '.\lightbulb-inactive.svg');
  $('#beaker').attr('src', '.\beaker-inactive.svg');
  $('#code-number').text('');
...
function triggerExpressionRequest() {
  ...
  $('#expression-route').attr('src', '.\expression-route-pending.svg');
  $('#expressions-route').attr('src', activePluralRouteImage[currentMode]);
  $('#code-number').text('');
  $('#code-description').text('');
  clearExpressionsText();
...
function triggerExpressionsRequest() {
  ...
  $('#expression-route').attr('src', activeSingularRouteImage[currentMode]);
  $('#expressions-route').attr('src', '.\expressions-route-pending.svg');
  $('#code-number').text('');
  $('#code-description').text('');
  clearExpressionText();
...
function animateMachine(animationDelay) {
  $('#beaker').attr('src', '.\beaker-pending.svg');
  $('#lightbulb').attr('src', '.\lightbulb-pending.svg');
  $('#sliders')
      .queue(function(next) { $(this).attr('src', '.\sliders-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\sliders-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\sliders-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\sliders-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\sliders-inactive.svg'); next(); });
  $('#buttons')
      .queue(function(next) { $(this).attr('src', '.\buttons-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\buttons-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\buttons-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\buttons-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', '.\buttons-inactive.svg'); next(); });
}
...
function animateGoodExpressionRequest(statusCode, statusDescription, response) {
  ...
  $('#expression-route').attr('src', '.\expression-route-green.svg');
...
function animateGoodExpressionsRequest(statusCode, statusDescription, response) {
  animateGoodRequest(statusCode, statusDescription);
  $('#expressions-route').attr('src', '.\expressions-route-green.svg');
...
function animateGoodRequest(statusCode, statusDescription) {
  $('#lightbulb').attr('src', '.\lightbulb-green.svg');
...
function animateBadExpressionRequest(statusCode, statusDescription) {
  ...
  $('#expression-route').attr('src', '.\expression-route-red.svg');
}

function animateBadExpressionsRequest(statusCode, statusDescription) {
  animateBadRequest(statusCode, statusDescription);
  $('#expressions-route').attr('src', '.\expressions-route-red.svg');
}

function animateBadRequest(statusCode, statusDescription) {
  $('#lightbulb').attr('src', '.\lightbulb-red.svg');
